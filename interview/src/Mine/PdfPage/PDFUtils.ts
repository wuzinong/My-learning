...
async function toCanvas(element: HTMLElement, width: number) {
  ...
  const canvas = await html2canvas(element, {
    ...
    onclone: (cloneDoc: any) => {
      //特殊处理某些元素
      cloneDoc.querySelector('header').style.display = 'block';
  ...
  document.body.appendChild(canvas); //out put the canvas obj for debug
...
export async function outputPDF({
  ...
  async function addFooter(
    ...
    const newFooter = footer.cloneNode(true) as any;
  ...
  function updatePos(eheight: any, top: any) {
  ...
  for (let i = 0; i < pages.length; ++i) {
    console.error(`共${pages.length}页， 生成第${i + 1}页`);
    // 根据分页位置新增图片
