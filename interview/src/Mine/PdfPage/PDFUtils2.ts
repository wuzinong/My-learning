...
async function myOutput({
  ...
  function updateNomalElPos(eheight, top) {
    if (
      top - (paegs.length > 0 ? paegs[paegs.length - 1] : 0) >
      originalPageHeight
    ) {
      //console.log({ top: top, one: one });
      paegs.push(
        (paegs.length > 0 ? paegs[paegs.length - 1] : 0) + originalPageHeight
      );
      //els.push(one);
      return true;
    }
  ...
  function updatePos(eheight, top, one) {
    ...
    // 距离当前页顶部的高度, 如果元素跨页，则将当前高度作为分页位置，推进
      ...
      top + eheight - (paegs.length > 0 ? paegs[paegs.length - 1] : 0) >
        originalPageHeight &&
      top != (paegs.length > 0 ? paegs[paegs.length - 1] : 0)
    ) {
      //console.log({ top: top, one: one });
  ...
  function traversingNodes(nodes) {
    for (let i = 0; i < nodes.length; ++i) {
      ...
      // 需要继续深入的节点
      const isPreviewItem =
        one.classList && one.classList.contains('preview-item');
      // 深度终点
      const isTableCol =
        one.classList && one.classList.contains('ant-table-row');
      // 特殊的富文本元素
      const isEditor =
        one.classList && one.classList.contains('pdf-editor-content');
